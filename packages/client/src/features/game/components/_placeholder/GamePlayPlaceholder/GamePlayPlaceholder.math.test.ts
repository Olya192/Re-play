// .math, потому что движок нужно рефакторить, здесь я написал логику только на +- чистые функции
// которые успел. По остальному оставил TODO в "движке"
import { computeItemLeft, computeItemTop, getItemSize } from './GamePlayPlaceholder';

const stage = { width: 600, height: 800 };

describe('getItemSize', () => {
  it('использует sizeRatio предмета, если он задан', () => {
    const item = { id: 1, kind: 'shard' as const, xPx: 0, spawnedAt: 0, sizeRatio: 0.5 };

    expect(getItemSize(item, stage)).toBe(300);
  });

  it('падает на дефолтный ITEM_SIZE_RATIO (1/6), если sizeRatio нет', () => {
    const item = { id: 1, kind: 'circle' as const, xPx: 0, spawnedAt: 0 };

    expect(getItemSize(item, stage)).toBe(100);
  });
});

describe('computeItemTop', () => {
  const item = { id: 1, kind: 'circle' as const, xPx: 0, spawnedAt: 1000, fallDurationMs: 1000 };
...
describe('computeItemLeft', () => {
  it('без горизонтальной скорости возвращает исходный x', () => {
    const item = { id: 1, kind: 'circle' as const, xPx: 200, spawnedAt: 0 };

    expect(computeItemLeft(item, 0, stage, 100)).toBe(200);
  });

  it('сдвигает по горизонтали со скоростью vxPxPerSec', () => {
    const item = { id: 1, kind: 'shard' as const, xPx: 200, spawnedAt: 0, vxPxPerSec: 100 };
  ...
  it('зажимает по левому краю (0)', () => {
    const item = { id: 1, kind: 'shard' as const, xPx: 100, spawnedAt: 0, vxPxPerSec: -100 };
