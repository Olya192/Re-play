import { ITEM_SIZE_RATIO, computeItemLeft, computeItemTop, getItemSize } from './GamePlayPlaceholder';
...
describe('computeItemTop', () => {
  const item = { id: 1, foodId: 1, kind: 'edible' as const, xPx: 0, spawnedAt: 1000, fallDurationMs: 1000 };
...
describe('computeItemLeft', () => {
  ...
  it('сдвигает по горизонтали со скоростью vxPxPerSec', () => {
    const item = { id: 1, foodId: 1, kind: 'shard' as const, xPx: 200, spawnedAt: 0, vxPxPerSec: 100 };
  ...
  it('зажимает по левому краю (0)', () => {
    const item = { id: 1, foodId: 1, kind: 'shard' as const, xPx: 100, spawnedAt: 0, vxPxPerSec: -100 };
